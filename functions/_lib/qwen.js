...
export async function qwen(env, body, stream = false, options = {}) {
  ...
  await diagnostic(env, {
    ...
    event: "provider_response_started",
    provider: credentials.provider,
    model: credentials.model,
    durationMs: Date.now() - startedAt,
    metadata: { stream, reasoning: responseReasoning.enabled },
  });
  return { response, reasoning: responseReasoning, telemetry: { provider: credentials.provider, model: credentials.model } };
}
